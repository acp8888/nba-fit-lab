...
```
 BBref / DARKO / Cleaning the Glass / PBPStats exports (manual CSV downloads)
        │
        ▼
...
3. **Staging models** (user writes, one .sql file per table):
   ...
   - `stg_ctg_team`: CTG team pages (garbage-time filtered) — four factors, shooting frequency/accuracy by zone (rim / short mid / long mid / corner 3 / non-corner 3), offense context (transition vs. halfcourt frequency and efficiency), for all 30 teams.
   - `stg_ctg_player`: CTG player shot profiles + on/off for ORL/NOP rosters (and league-wide percentile columns as exported).
   - `stg_pbp_lineups` from PBPStats (5-man lineup detail — CTG doesn't publish full 5-man lineups, so PBPStats/BBref stay the lineup source; CTG becomes the source of truth for shot-profile and context features).
...
4. **Marts** (the analysis contract):
   - `mart_lineup_features`: one row per lineup-stint aggregate, with engineered fit features — `n_shooters` (players ≥ league-median 3PA frequency AND ≥ ~36% on volume, using CTG's garbage-time-filtered player shooting), `has_rim_protector` (CTG opponent-rim-accuracy on/off delta or block% proxy), `n_creators` (self-created shot share or high AST+USG proxy), `spacing_score` (sum of teammates' 3-pt gravity proxies), plus net rating and possessions. Derive the thresholds from staging data, not vibes.
   - `mart_team_style`: one row per NBA team-season, built primarily from `stg_ctg_team` (garbage-time filtered): pace, 3PA rate, rim frequency, long-mid frequency, TOV% forced, OREB%, transition frequency and efficiency, halfcourt efficiency, plus a size proxy from BBref. (All 30 teams — needed for clustering. CTG's zone frequencies make the archetypes much sharper than raw per-100 stats.)
...
- **A2 — Scarcity regression:** minutes-weighted regression of lineup net rating on `n_shooters`, `has_rim_protector`, `n_creators` (+ interactions), Magic and Pelicans separately, league pooled as a baseline. Teach: weighting by possessions, small-sample honesty (lineup data is noisy — report intervals, consider empirical-Bayes shrinkage toward team mean), the difference between "statistically significant" and "big enough to matter," and *why garbage time distorts fit analysis* (blowout minutes are played by exactly the low-fit lineups you're studying — CTG's filtering is a methods point worth a footnote in Post 2).
...
2. Ship the small Parquet marts the notebook needs alongside the site (or public-read `marts/` prefix / R2 mirror); confirm the WASM app loads them over HTTP.
   **CTG licensing guardrail:** CTG is paid subscription content. Never put raw CTG exports (or thinly-transformed CTG tables) in the public repo, a public bucket, or the WASM site's data files. Charts and derived aggregates in posts are standard practice — credit "data via Cleaning the Glass" — but the public interactive companion must run only on BBref/PBPStats-derived marts. Keep CTG-derived marts under a private prefix; split marts into `marts/public/` and `marts/private/` to make this structural rather than a thing you have to remember.
...
| Cleaning the Glass — Team pages | For all 30 teams: Four Factors, Shooting Frequency & Accuracy (offense and defense), Offense/Defense Context (transition vs. halfcourt). Use the CSV/export option on each table; grab both offense and defense views | stg_ctg_team → mart_team_style |
| Cleaning the Glass — Player pages | ORL + NOP rosters: Shooting Frequency/Accuracy, On/Off (Efficiency + Shooting) per player | stg_ctg_player → fit features |
| PBPStats.com | 5-man lineup exports for ORL/NOP (CTG doesn't publish full lineups) | stg_pbp_lineups |
| NBA.com/stats (manual) | Anything missing above (e.g., tracking touches) — optional | enrichment |

CTG export tip: exports are per-table and per-season, so the 30-team style pull is ~3–4 tables × 30 clicks if done from team pages — but CTG's league-wide "Team Tables" views give you all 30 teams in one table per stat group. Use those; it's ~5 exports total instead of ~100.
