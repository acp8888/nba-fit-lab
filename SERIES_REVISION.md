...
2. **C (generalize beyond n=2) — DONE & POOLED (n=20, both seasons), in Post 1.** Pooling 2024-25 **deflated** the 2025-26 result: complement builds over-perform talent (**+1.5**), duplicates under-perform (**−1.5**) — a ~3-pt gap in the right direction, corr(n_bdn, fit) **−0.32**, but **UNDERPOWERED** (n=20, p≈0.2). The simpler shooter-COUNT version **washed out** (r +0.69 → +0.27) — it was small-sample noise. Framed as a *weak edge dwarfed by talent*, consistent with Post 2. Uses new `_lab.load_team_seasons()` (BBref NRtg + DARKO both seasons). Full Mosley-tenure team pull would firm it.
